...
const Uploading = () => {
  ...
  const handleFileUpload = async () => {
    ...
    await axios
      ...
      .then((response) => {
        ...
        if (response.status === 200) navigate(ROUTES.customers);
      });
  };
  useEffect(() => {
    if (selectedFile) {
  ...
  return (
    <Flex vertical align="center" justify="center" gap={32} className="mt-8 bg-white py-[148px] rounded-md">
      <Flex className="rounded-md border-[1px] border-[#CED4DA] p-8" gap="middle" align="center">
        ...
        <Space direction="vertical">
          ...
          {progress && <Progress percent={Math.ceil(progress * 100)} status="active" />}
        </Space>
      </Flex>
